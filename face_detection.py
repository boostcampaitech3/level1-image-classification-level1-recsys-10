    ...
    try:
        for idx in tqdm(range(0, path_df.shape[0])):
            ...
            for (x, y, x2, y2), conf in zip(faces, confidences):
                ...
                if (x < 384 and y < 512 and x2 < 384 and y2 < 512) and (x2 - x) * (
                    y2 - y
                ) > data_size:
                    # conf 85% 이상인 bbox
                    if conf > 0.85:
                        ...
                        # (512, 384, 3)
                        img2 = img[
                            max(y - 10, 0) : min(y2 + 10, 512),
                            max(x - 10, 0) : min(x2 + 10, 384),
                            :,
                        ]
                        cv2.imwrite(IMAGEPATH, img2)
                        break
...
def eval(data_size):
    ...
    # path 열에 경로 저장
    path_df["path"] = path_df["ImageID"].apply(
        lambda x: os.path.join(EVAL_IMAGES_PATH, x)
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        default="train",
        help="data type, train or eval (default='train')",
    )
    parser.add_argument(
        "--size",
        type=int,
        default="10000",
        help="data size, width*hegiht (default='10000')",
    )
